...
const ReadingProgress = ({ progress }: { progress: number }) => (
  <div className="fixed top-0 left-0 right-0 z-[100] h-[3px] bg-transparent">
    <div
      className="h-full bg-c-red transition-all duration-75 ease-out"
...
const TableOfContents = ({ content }: { content: string }) => {
  ...
  return (
    <nav className="p-5 bg-surface border border-border rounded-xl">
      <p className="text-[11px] font-bold tracking-widest uppercase text-muted mb-4">
        On this page
      </p>
      <ul className="flex flex-col gap-0.5">
        {headings.map((h, i) => (
          <li key={i} style={{ paddingLeft: `${(h.level - 1) * 12}px` }}>
            <a
              href={`#${h.id}`}
              className="block px-2 py-1.5 rounded-md text-[13px] text-muted hover:text-foreground hover:bg-surface-2 transition-colors duration-150 leading-snug"
            >
              {h.text}
            </a>
          </li>
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button
          ...
          className="group flex items-center justify-center w-9 h-9 rounded-lg bg-surface border border-border hover:border-blue-500/50 hover:bg-blue-500/10 transition-all duration-200"
          title="Share on Facebook"
        >
          <svg className="w-4 h-4 text-muted group-hover:text-blue-400 transition-colors" fill="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className="group flex items-center justify-center w-9 h-9 rounded-lg bg-surface border border-border hover:border-white/30 hover:bg-white/10 transition-all duration-200"
          title="Share on X"
        >
        ...
        <button
          ...
          className="group flex items-center justify-center w-9 h-9 rounded-lg bg-surface border border-border hover:border-blue-400/50 hover:bg-blue-400/10 transition-all duration-200"
        ...
        <button
          ...
          className="group flex items-center justify-center w-9 h-9 rounded-lg bg-surface border border-border hover:border-green-500/50 hover:bg-green-500/10 transition-all duration-200"
          title="Share on WhatsApp"
        >
          <svg className="w-4 h-4 text-muted group-hover:text-green-400 transition-colors" fill="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className="group flex items-center justify-center w-9 h-9 rounded-lg bg-surface border border-border hover:border-orange-500/50 hover:bg-orange-500/10 transition-all duration-200"
          title="Share on Reddit"
        >
          <svg className="w-4 h-4 text-muted group-hover:text-orange-400 transition-colors" fill="currentColor" viewBox="0 0 24 24">
        ...
        {typeof navigator !== 'undefined' && typeof navigator.share === 'function' && (
          <button
            onClick={nativeShare}
            className="group flex items-center justify-center w-9 h-9 rounded-lg bg-surface border border-border hover:border-emerald-500/50 hover:bg-emerald-500/10 transition-all duration-200"
            title="Share via..."
          >
            <svg className="w-4 h-4 text-muted group-hover:text-emerald-400 transition-colors" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <div className="relative">
          <button
            ...
            className={`group flex items-center justify-center w-9 h-9 rounded-lg border transition-all duration-200 ${
              copied
                ? 'bg-c-green/15 border-c-green/50 text-link'
                : 'bg-surface border-border hover:border-foreground/30 hover:bg-surface-2'
            }`}
            title="Copy link"
          >
            {copied ? (
              ...
              <svg className="w-4 h-4 text-muted group-hover:text-foreground transition-colors" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
            )}
          </button>
          {(showTooltip || copied) && (
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 rounded text-[10px] bg-foreground border border-border text-background whitespace-nowrap">
...
const BlogPostClient = () => {
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5">
          <div className="w-11 h-11 rounded-full border-2 border-border border-t-c-red animate-spin" />
          <p className="text-muted text-sm tracking-wider">Loading article…</p>
        </div>
        <Footer />
      </div>
    )
  ...
  if (!post) {
    return (
      <div className="min-h-screen flex flex-col">
        <Navbar />
        <div className="flex-1 flex flex-col items-center justify-center text-center px-6 gap-5 pt-24">
          <span className="display-serif text-[120px] leading-none text-foreground">
            404
          </span>
          <h1 className="display-serif text-3xl text-foreground">Article not found</h1>
          <p className="text-muted max-w-sm">
            The post you&apos;re looking for doesn&apos;t exist or has been moved.
          </p>
          <Link
            href="/blog"
            className="pop-btn mt-2 inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-foreground border border-foreground text-sm text-background hover:opacity-90 transition-all duration-200"
          >
            ← Back to Blog
          </Link>
        </div>
